...
var w2ui = w2ui || {};
var w2obj = w2obj || {};
// expose object to be able to overwrite default functions
...
var w2utils = (function() {
	...
	function age(timeStr) {
		...
		if (w2utils.isInt(timeStr))
			d1 = new Date(Number(timeStr));
		// for unix timestamps
		var tmp = String(timeStr).split('-');
		if (tmp.length == 3)
			d1 = new Date(tmp[0], Number(tmp[1]) - 1, tmp[2]);
		// yyyy-mm-dd
		var tmp = String(timeStr).split('/');
		if (tmp.length == 3)
			d1 = new Date(tmp[2], Number(tmp[0]) - 1, tmp[1]);
		// mm/dd/yyyy
		if (d1 == 'Invalid Time')
			return '';
	...
	function formatDate(dateStr, format) {// IMPORTANT dateStr HAS TO BE valid JavaScript Date String
		...
		if (w2utils.isInt(dateStr))
			dt = new Date(Number(dateStr));
		// for unix timestamps
		var tmp = String(dateStr).split('-');
		if (tmp.length == 3)
			dt = new Date(tmp[0], Number(tmp[1]) - 1, tmp[2]);
		// yyyy-mm-dd
		var tmp = String(dateStr).split('/');
		if (tmp.length == 3)
			dt = new Date(tmp[2], Number(tmp[0]) - 1, tmp[1]);
		// mm/dd/yyyy
		if (dt == 'Invalid Date')
			return '';
		...
		.replace(/(^|[^a-z$])d/g, '$1' + date);
		// only y's that are not preceeded by a letter
	}
	...
	function formatTime(dateStr, format) {// IMPORTANT dateStr HAS TO BE valid JavaScript Date String
		...
		return format.toLowerCase().replace('am', type).replace('pm', type).replace('hh', hour).replace('h24', h24).replace('mm', min).replace('mi', min).replace('ss', sec).replace(/(^|[^a-z$])h/g, '$1' + hour)// only y's that are not preceeded by a letter
		.replace(/(^|[^a-z$])m/g, '$1' + min)// only y's that are not preceeded by a letter
		.replace(/(^|[^a-z$])s/g, '$1' + sec);
		// only y's that are not preceeded by a letter
	}
	...
	function transition(div_old, div_new, type, callBack) {
		...
		function cross(property, value, none_webkit_value) {
			...
			// jQuery no longer supports $.browser - RR
	...
	function lock(box, msg, showSpinner) {
		...
		setTimeout(function() {
			...
			setTimeout(function() {
				...
				if (showSpinner === true)
					msg = '<div class="w2ui-spinner" ' + (!msg ? 'style="width: 30px; height: 30px"' : '') + '></div>' + msg;
				mess.html(msg).css({
	...
	function scrollBarSize() {
		...
		if (String(navigator.userAgent).indexOf('MSIE') >= 0)
			tmp.scrollBarSize = tmp.scrollBarSize / 2;
		// need this for IE9+
		return tmp.scrollBarSize;
	}
		...
		for (var h = this.handlers.length - 1; h >= 0; h--) {
			...
			if ((item.event.type == eventData.type || item.event.type == '*') && (item.event.target == eventData.target || item.event.target == null) && (item.event.execute == eventData.phase || item.event.execute == '*' || item.event.phase == '*')) {
				...
				if (args.length == 2) {
					...
					// old way for back compatibility
				} else {
				...
				if (eventData.isStopped === true || eventData.stop === true)
					return eventData;
				// back compatibility eventData.stop === true
			}
		}
		// main object events
	...
	$.fn.w2marker = function(str) {
		if (str == '' || typeof str == 'undefined') {// remove marker
			return $(this).each(function(index, el) {
				el.innerHTML = el.innerHTML.replace(/\<span class=\"w2ui\-marker\"\>(.*)\<\/span\>/ig, '$1');
				// unmark
			});
		} else {// add marker
			return $(this).each(function(index, el) {
	...
	$.fn.w2overlay = function(html, options) {
		...
		function fixSize() {
			...
			if ($('#w2ui-overlay > div').length > 0) {
				...
				if (h > max)
					$('#w2ui-overlay> div').height(max).width(w + w2utils.scrollBarSize()).css({
						'overflow-y' : 'auto'
					});
				// check width
				...
				if (w > max)
					$('#w2ui-overlay> div').width(max).css({
						'overflow-x' : 'auto'
					});
				// onShow event
				if ( typeof options.onShow == 'function')
...
(function() {
	var w2grid = function(options) {
		...
		this.name = null;
		this.box = null;
		// HTML element that hold this element
		this.header = '';
		this.url = '';
		this.columns = [];
		// { field, caption, size, attr, render, hidden, gridMinWidth, [editable: {type, inTag, outTag, style, items}] }
		this.columnGroups = [];
		// { span: int, caption: 'string', master: true/false }
		this.records = [];
		// { recid: int(requied), field1: 'value1', ... fieldN: 'valueN', style: 'string', editable: true/false, summary: true/false, changed: true/false, changes: object }
		this.summary = [];
		// arry of summary records, same structure as records array
		this.searches = [];
		// { type, caption, field, inTag, outTag, default, items, hidden }
		this.searchData = [];
		this.sortData = [];
		this.postData = {};
		this.toolbar = {};
		// if not empty object; then it is toolbar object
		...
		// for infinite scroll
		this.fixedBody = true;
		// if false; then grid grows with data
		...
		this.limit = 100;
		this.offset = 0;
		// how many records to skip (for infinite scroll) when pulling from server
		this.style = '';
		this.ranges = [];
		...
		this.onSearch = null;
		this.onChange = null;
		// called when editable record is changed
		this.onExpand = null;
		this.onCollapse = null;
		this.onError = null;
		this.onKeydown = null;
		this.onToolbar = null;
		// all events from toolbar
	...
	$.fn.w2grid = function(method) {
		if ( typeof method === 'object' || !method) {
			...
			if (object.onExpand != null)
				object.show.expandColumn = true;
			...
			if (object.records.length > 0)
				object.buffered = object.records.length;
			// add searches
			for (var c in object.columns) {
				var col = object.columns[c];
				if ( typeof col.searchable == 'undefined' || object.getSearch(col.field) != null)
					continue;
	...
	w2grid.prototype = {
		...
		buttons : {
			...
			'search' : {
				type : 'html',
				id : 'search',
				html : '<div class="w2ui-icon icon-search-down w2ui-search-down" title="' + 'Select Search Field' + '" ' + 'onclick="var obj = w2ui[$(this).parents(\'div.w2ui-grid\').attr(\'name\')]; obj.searchShowFields(this);"></div>'
			},
			'search-go' : {
				type : 'check',
				id : 'search-advanced',
				caption : 'Search...',
				hint : 'Open Search Fields'
			},
			'add' : {
				type : 'button',
				id : 'add',
			...
			for (var i = 0; i < this.records.length; i++) {
				...
				for (var o in obj) {
					...
					if (String(o).indexOf('.') != -1)
						val = this.parseField(this.records[i], o);
					if (obj[o] != val)
						match = false;
				}
				if (match && returnIndex !== true)
		...
		getSearchData : function(field) {
			for (var s in this.searchData) {
				if (this.searchData[s].field == field)
					return this.searchData[s];
			}
			return null;
		},
		...
		localSort : function(silent) {
			...
			this.records.sort(function(a, b) {
				...
				for (var s in obj.sortData) {
					...
					if (aa > bb)
						ret = (obj.sortData[s].direction == 'asc' ? 1 : -1);
					if (aa < bb)
						ret = (obj.sortData[s].direction == 'asc' ? -1 : 1);
					if ( typeof aa != 'object' && typeof bb == 'object')
						ret = -1;
					if ( typeof bb != 'object' && typeof aa == 'object')
						ret = 1;
					if (ret != 0)
						break;
				}
				...
				for (var r in this.records) {
					...
					for (var s in this.searchData) {
						...
						switch (sdata.operator) {
							case 'is':
								if (rec[search.field] == sdata.value)
									fl++;
								// do not hide record
								if (search.type == 'date') {
								...
								break;
							case 'between':
								if (search.type == 'int' && parseInt(rec[search.field]) >= parseInt(val2) && parseInt(rec[search.field]) <= parseInt(val3))
									fl++;
								if (search.type == 'float' && parseFloat(rec[search.field]) >= parseFloat(val2) && parseFloat(rec[search.field]) <= parseFloat(val3))
									fl++;
								if (search.type == 'date') {
							...
							case 'contains':
								...
								break;
							case 'ends with':
								if (val1.indexOf(val2) == val1.length - val2.length)
									fl++;
								// do not hide record
								break;
						}
		...
		addRange : function(ranges) {
			...
			// if it is selection
			for (var r in ranges) {
				if ( typeof ranges[r] != 'object')
					ranges[r] = {
						name : 'selection'
					};
				if (ranges[r].name == 'selection') {
					if (this.show.selectionBorder === false)
						continue;
		...
		refreshRanges : function() {
			...
			function mouseMove(event) {
				...
				if (tmp.tagName != 'TD')
					tmp = $(tmp).parents('td')[0];
				if ( typeof $(tmp).attr('col') != 'undefined')
		...
		unselect : function() {
			...
			for (var a = 0; a < arguments.length; a++) {
				...
				if (this.selectType == 'row') {
					...
					if (el.length != 0)
						el[0].style.cssText = 'height: ' + this.recordHeight + 'px; ' + el.attr('custom_style');
			...
			if (!url) {
				if (this.searchData.length == 0) {
					...
					for (var i = 0; i < this.last.searchIds.length; i++) {
						...
						if (this.selectType != 'row')
							this.records[this.last.searchIds[i]].selectedColumns = cols.slice();
					}
					this.refresh();
				}
			...
			for (var i in this.records) {
				...
				if (rec.selected === true) {
					...
					if (this.selectType != 'row') {
						...
						for (var c in cols)
						tmp.find(' > td[col=' + cols[c] + ']').removeClass('w2ui-selected');
						rec.selectedColumns = [];
					}
				}
			}
			...
			if (this.selectType == 'row') {
				...
				for (var s in sel) {
					...
					for (var c in rec.selectedColumns) {
						ret.push({
							recid : rec.recid,
							index : parseInt(sel[s]),
							column : rec.selectedColumns[c]
						});
					}
				}
				return ret;
		...
		searchClose : function() {
			...
			if (this.searches.length == 0)
				return;
			if (this.toolbar)
				this.toolbar.uncheck('search-advanced')
			// hide search
			if ($('#w2ui-overlay .w2ui-grid-searches').length > 0)
				$().w2overlay();
		},

		searchShowFields : function(el) {
			if ( typeof el == 'undefined')
				el = $('#grid_' + this.name + '_search_all');
			...
			for (var s = -1; s < this.searches.length; s++) {
				...
				if (s == -1) {
					...
					if (this.searches[s].hidden === true)
						continue;
				}
		...
		reset : function(noRefresh) {
			...
			// initial sort
			if (this.last.sortData != null)
				this.sortData = this.last.sortData;
		...
		skip : function(offset) {
			...
			if (url) {
				...
				if (this.offset < 0 || !w2utils.isInt(this.offset))
					this.offset = 0;
		...
		reload : function(callBack) {
			var url = ( typeof this.url != 'object' ? this.url : this.url.get);
			if (url) {
				if (this.last.xhr_offset > 0 && this.last.xhr_offset < this.buffered)
					this.last.xhr_offset = this.buffered;
		...
		request : function(cmd, add_params, url, callBack) {
			...
			var params = {};
			if (!w2utils.isInt(this.offset))
				this.offset = 0;
			if (!w2utils.isInt(this.last.xhr_offset))
				this.last.xhr_offset = 0;
			// add list params
			params['cmd'] = cmd;
			...
			if (params.cmd == 'save-records') {
				if ( typeof eventData.url == 'object')
					url = eventData.url.save;
				xhr_type = 'PUT';
				// so far it is always update
			}
			if (params.cmd == 'delete-records') {
				if ( typeof eventData.url == 'object')
					url = eventData.url.remove;
				xhr_type = 'DELETE';
			}
			if (!w2utils.settings.RESTfull)
				xhr_type = 'POST';
		...
		requestComplete : function(status, cmd, callBack) {
			...
			// event before
			var event_name = 'load';
			if (this.last.xhr_cmd == 'save-records')
				event_name = 'saved';
			if (this.last.xhr_cmd == 'delete-records')
				event_name = 'deleted';
			var eventData = this.trigger({
				phase : 'before',
				target : this.name,
		...
		editField : function(recid, column, value, event) {
			...
			if (edit.type == 'enum')
				console.log('ERROR: Grid\'s inline editing does not support enum field type.');
			if (edit.type == 'list' || edit.type == 'select')
				console.log('ERROR: Grid\'s inline editing does not support list/select field type.');
			if ( typeof edit.inTag == 'undefined')
				edit.inTag = '';
			if ( typeof edit.outTag == 'undefined')
				edit.outTag = '';
			if ( typeof edit.style == 'undefined')
				edit.style = '';
			if ( typeof edit.items == 'undefined')
				edit.items = [];
			...
			if ($.inArray(col.render, ['number', 'int', 'float', 'money', 'percent']) != -1)
			...
			el.find('input').w2field(edit.type, edit).on('click', function(event) {
				...
				if (obj.parseField(rec, col.field) != this.value) {
					...
					var eventData2 = obj.trigger({
						...
						recid : recid,
						column : column,
						value_new : this.value,
						value_previous : (rec.changes ? rec.changes[col.field] : obj.parseField(rec, col.field)),
				...
				function nextCell(check) {
					...
					if (obj.columns.length == newCheck)
						return check;
					if (obj.columns[newCheck].hidden)
						return nextCell(newCheck);
					return newCheck;
				}
				...
				function prevCell(check) {
					...
					if (newCheck < 0)
						return check;
					if (obj.columns[newCheck].hidden)
						return prevCell(newCheck);
					return newCheck;
				}

				function nextRow(check) {
					var newCheck = check + 1;
					if (obj.records.length == newCheck)
						return check;
					return newCheck;
				}
				...
				if ( typeof recs[0] != 'object') {
					...
					for (var r in recs) {
						...
						if (ind != null && fld != 'recid') {
							...
							if (this.records[ind].changed)
								this.records[ind].changes[fld] = '';
						}
					}
					this.refresh();
				}
		...
		click : function(recid, event) {
			...
			if (event.shiftKey && sel.length > 0) {
				...
				for (var i = start; i <= end; i++) {
					if (this.searchData.length > 0 && !url && $.inArray(i, this.last.searchIds) == -1)
						continue;
					if (this.selectType == 'row') {
				...
				if (((!event.ctrlKey && !event.shiftKey && !event.metaKey) || !this.multiSelect) && !this.showSelectColumn) {
					...
					if (this.selectType != 'row' && $.inArray(column, record.selectedColumns) == -1)
						flag = false;
					if (sel.length > 300)
						this.selectNone();
					else
						this.unselect.apply(this, sel);
					if (flag === true) {
						this.unselect({
							recid : recid,
							column : column
		...
		keydown : function(event) {
			...
			switch (event.keyCode) {
				...
				case 13:
					// enter
					if (this.selectType == 'row') {
						if (recEL.length <= 0 || obj.show.expandColumn !== true)
							break;
				...
				case 37:
					...
					if (this.selectType == 'row') {
						if (recEL.length <= 0 || rec.expanded !== true)
							break;
						obj.set(recid, {
							expanded : false
						}, true);
						...
						if (prev != columns[0]) {
							if (event.shiftKey) {
								...
								if (columns.indexOf(this.last.sel_col) == 0 && columns.length > 1) {
									for (var i in sel) {
										if (tmp.indexOf(sel[i].recid) == -1)
											tmp.push(sel[i].recid);
										unSel.push({
											recid : sel[i].recid,
				...
				case 39:
					// right
					if (this.selectType == 'row') {
						if (recEL.length <= 0 || rec.expanded === true || obj.show.expandColumn !== true)
							break;
			...
			function nextRow(ind) {
				...
				|| (obj.last.searchIds.length > 0 && ind < obj.last.searchIds[obj.last.searchIds.length - 1])) {
					...
					if (obj.last.searchIds.length > 0) {
						while (true) {
							if ($.inArray(ind, obj.last.searchIds) != -1 || ind > obj.records.length)
								break;
							ind++;
						}
					}
			...
			function prevRow(ind) {
				...
				|| (obj.last.searchIds.length > 0 && ind > obj.last.searchIds[0])) {
					...
					if (obj.last.searchIds.length > 0) {
						while (true) {
							if ($.inArray(ind, obj.last.searchIds) != -1 || ind < 0)
								break;
							ind--;
						}
					}
		...
		scrollIntoView : function(ind) {
			...
			if (records.height() > this.recordHeight * (len > 0 ? len : this.records.length))
				return;
			if (len > 0)
				ind = this.last.searchIds.indexOf(ind);
			// if seach is applied
		...
		expand : function(recid) {
			...
			if ($('#grid_' + this.name + '_rec_' + id + '_expanded_row').length > 0)
				return false;
			if (rec.expanded == 'none')
				return false;
			// insert expand row
			var tmp = 1 + (this.show.selectColumn ? 1 : 0);
			var addClass = '';
			// ($('#grid_'+this.name +'_rec_'+ w2utils.escapeId(recid)).hasClass('w2ui-odd') ? 'w2ui-odd' : 'w2ui-even');
		...
		copy : function() {
			...
			if ( typeof sel[0] == 'object') {// cell copy
				...
				var recs = [];
				for (var s in sel) {
					if (sel[s].column < minCol)
						minCol = sel[s].column;
					if (sel[s].column > maxCol)
						maxCol = sel[s].column;
					if (recs.indexOf(sel[s].index) == -1)
						recs.push(sel[s].index);
				}
				recs.sort();
				for (var r in recs) {
					var ind = recs[r];
		...
		refresh : function() {
			...
			for (var s in this.searches) {
				if (this.searches[s].field == this.last.field)
					this.last.caption = this.searches[s].caption;
			}
			if (this.last.multi) {
			...
			if (this.last.selected.length > 0)
				for (var s in this.last.selected) {
		...
		render : function(box) {
			...
			if (!this.box)
				return;
			if (this.last.sortData == null)
				this.last.sortData = this.sortData;
			// event before
			var eventData = this.trigger({
				phase : 'before',
				target : this.name,
			...
			if (this.selectType != 'row')
				$(this.box).addClass('w2ui-ss');
			if ($(this.box).length > 0)
				$(this.box)[0].style.cssText += this.style;
			// init toolbar
			this.initToolbar();
			if (this.toolbar != null)
				this.toolbar.render($('#grid_'+ this.name +'_toolbar')[0]);
			// init footer
			$('#grid_' + this.name + '_footer').html(this.getFooterHTML());
			// refresh
			this.refresh();
			// show empty grid (need it)
			this.reload();
			...
			function mouseStart(event) {
				if ($(event.target).parents().hasClass('w2ui-head') || $(event.target).parents().hasClass('w2ui-empty-record'))
					return;
				if (obj.last.move && obj.last.move.type == 'expand')
					return;
				obj.last.move = {
					x : event.screenX,
					y : event.screenY,
			...
			function mouseMove(event) {
				if (!obj.last.move || obj.last.move.type != 'select')
					return;
				obj.last.move.divX = (event.screenX - obj.last.move.x);
				obj.last.move.divY = (event.screenY - obj.last.move.y);
				if (Math.abs(obj.last.move.divX) <= 1 && Math.abs(obj.last.move.divY) <= 1)
					return;
				// only if moved more then 1px
				...
				if (obj.last.move.range == tmp)
					return;
				obj.last.move.range = tmp;
				for (var i = ind1; i <= ind2; i++) {
					if (obj.last.searchIds.length > 0 && obj.last.searchIds.indexOf(i) == -1)
						continue;
					if (obj.selectType != 'row') {
						if (col1 > col2) {
							var tmp = col1;
				...
				if (obj.selectType != 'row') {
					...
					for (var ns in newSel) {
						var flag = false;
						for (var s in sel)
						if (newSel[ns].recid == sel[s].recid && newSel[ns].column == sel[s].column)
							flag = true;
						if (!flag)
							tmp.push({
					...
					if (obj.multiSelect) {
						var sel = obj.getSelection();
						for (var ns in newSel)
						if (sel.indexOf(newSel[ns]) == -1)
							obj.select(newSel[ns]);
						// add more items
						for (var s in sel)
						if (newSel.indexOf(sel[s]) == -1)
							obj.unselect(sel[s]);
						// remove items
					}
				}
		...
		destroy : function() {
			...
			// clean up
			if ( typeof this.toolbar == 'object' && this.toolbar.destroy)
		...
		columnOnOff : function(el, event, field, value) {
			...
			for (var r in this.records) {
				if (this.records[r].expanded === true)
					this.records[r].expanded = false
			}
			// show/hide
			var hide = true;
		...
		initSearches : function() {
			...
			for (var s in this.searches) {
					...
					case 'list':
						...
						for (var i in search.items) {
							if ($.isPlainObject(search.items[i])) {
								...
								if ( typeof val == 'undefined' && typeof search.items[i].value != 'undefined')
									val = search.items[i].value;
								if ( typeof txt == 'undefined' && typeof search.items[i].caption != 'undefined')
									txt = search.items[i].caption;
								if (val == null)
									val = '';
				...
				if (sdata != null) {
					...
					if (!$.isArray(sdata.value)) {
						if ( typeof sdata.value != 'udefined')
			...
			$(this.box).find('.w2ui-resizer').off('click').on('click', function(event) {
				...
				for (var c in obj.columns) {
					if ( typeof obj.columns[c].sizeOriginal == 'undefined')
						obj.columns[c].sizeOriginal = obj.columns[c].size;
		...
		resizeRecords : function() {
			...
			if (body.width() < $(records).find('>table').width())
				bodyOverflowX = true;
			if (body.height() - columns.height() < $(records).find('>table').height() + ( bodyOverflowX ? w2utils.scrollBarSize() : 0))
				bodyOverflowY = true;
			if (!this.fixedBody) {
				bodyOverflowY = false;
				bodyOverflowX = false;
				...
				if (this.fixedBody) {
					for (var di = this.buffered; di <= max; di++) {
						...
						if (this.show.lineNumbers)
							html += '<td class="w2ui-col-number"></td>';
						if (this.show.selectColumn)
							html += '<td class="w2ui-grid-data w2ui-col-select"></td>';
						if (this.show.expandColumn)
							html += '<td class="w2ui-grid-data w2ui-col-expand"></td>';
						var j = 0;
			...
			for (var i = 0; i < this.columns.length; i++) {
				...
				if (col.hidden)
					continue;
				if ( typeof col.min == 'undefined')
					col.min = 20;
				if (parseInt(col.sizeCalculated) < parseInt(col.min))
					col.sizeCalculated = col.min + 'px';
				if (parseInt(col.sizeCalculated) > parseInt(col.max))
					col.sizeCalculated = col.max + 'px';
		...
		getSearchesHTML : function() {
			...
			for (var i = 0; i < this.searches.length; i++) {
				...
				if ( typeof s.inTag == 'undefined')
					s.inTag = '';
				if ( typeof s.outTag == 'undefined')
					s.outTag = '';
		...
		getColumnsHTML : function() {
			...
			function getGroups() {
				...
				if (obj.columnGroups[obj.columnGroups.length - 1].caption != '')
					obj.columnGroups.push({
						caption : ''
					});
				...
				for (var i = 0; i < obj.columnGroups.length; i++) {
					...
					if ( typeof colg.span == 'undefined' || colg.span != parseInt(colg.span))
						colg.span = 1;
					if ( typeof colg.colspan != 'undefined')
						colg.span = colg.colspan;
					if (colg.master === true) {
						var sortStyle = '';
						for (var si in obj.sortData) {
							if (obj.sortData[si].field == col.field) {
								if (obj.sortData[si].direction == 'asc')
									sortStyle = 'w2ui-sort-down';
								if (obj.sortData[si].direction == 'desc')
									sortStyle = 'w2ui-sort-up';
							}
						}
						var resizer = "";
			...
			function getColumns(master) {
				...
				for (var i = 0; i < obj.columns.length; i++) {
					...
					if ( typeof obj.columnGroups[ii - 1] != 'undefined')
						var colg = obj.columnGroups[ii - 1];
					if (col.hidden)
						continue;
					var sortStyle = '';
					for (var si in obj.sortData) {
		...
		scroll : function(event) {
			...
			var obj = this;
			var records = $('#grid_' + this.name + '_records');
			if (this.records.length == 0 || records.length == 0 || records.height() == 0)
				return;
			if (this.buffered > 300)
				this.show_extra = 30;
			else
			...
			if (String(tr1.next().prop('id')).indexOf('_expanded_row') != -1)
				tr1.next().remove();
			if (String(tr2.prev().prop('id')).indexOf('_expanded_row') != -1)
				tr2.prev().remove();
			...
			if (first < start || first == 1 || this.last.pull_refresh) {// scroll down
				...
				while (true) {
					...
					if (tmp.attr('line') == 'bottom')
						break;
					if (parseInt(tmp.attr('line')) < start)
						tmp.remove();
					else
						break;
				}
				...
				for (var i = parseInt(rec_start) + 1; i <= end; i++) {
					if (!this.records[i - 1])
						continue;
					if (this.records[i - 1].expanded === true)
						this.records[i - 1].expanded = false;
				...
				while (true) {
					var tmp = records.find('#grid_' + this.name + '_rec_bottom').prev();
					if (tmp.attr('line') == 'top')
						break;
					if (parseInt(tmp.attr('line')) > end)
						tmp.remove();
					else
						break;
				}
			...
			// check for grid end
			if (this.buffered >= this.total - this.offset)
				$('#grid_' + this.name + '_rec_more').hide();
			return;

			function markSearch() {
				// mark search
				if (obj.markSearchResults === false)
					return;
		...
		getRecordHTML : function(ind, lineNum, summary) {
			...
			if (ind == -1) {
				rec_html += '<tr line="0">';
				if (this.show.lineNumbers)
					rec_html += '<td class="w2ui-col-number" style="height: 0px;"></td>';
				if (this.show.selectColumn)
					rec_html += '<td class="w2ui-col-select" style="height: 0px;"></td>';
				if (this.show.expandColumn)
					rec_html += '<td class="w2ui-col-expand" style="height: 0px;"></td>';
			...
			if (summary !== true) {
				if (this.searchData.length > 0 && !url) {
					if (ind >= this.last.searchIds.length)
						return '';
					ind = this.last.searchIds[ind];
					record = this.records[ind];
				} else {
					if (ind >= this.records.length)
						return '';
					record = this.records[ind];
				}
			} else {
				if (ind >= this.summary.length)
					return '';
			...
			if (this.show.expandColumn) {
				...
				if (record.expanded === true)
					...
					tmp_img = '+';
				if (record.expanded == 'none')
					tmp_img = '';
				if (record.expanded == 'spinner')
					tmp_img = '<div class="w2ui-spinner" style="width: 16px; margin: -2px 2px;"></div>';
			...
			while (true) {
				...
				if (col.hidden) {
					col_ind++;
					if ( typeof this.columns[col_ind] == 'undefined')
						break;
					else
						continue;
				}
				...
				if ( typeof col.render == 'string') {
					...
					if ($.inArray(tmp[0], ['number', 'int', 'float', 'money', 'percent']) != -1)
						addStyle = 'text-align: right';
					if ($.inArray(tmp[0], ['date']) != -1)
						addStyle = 'text-align: right';
				}
				var isCellSelected = false;
				if (record.selected && $.inArray(col_ind, record.selectedColumns) != -1)
					isCellSelected = true;
		...
		getCellHTML : function(ind, col_ind, summary) {
			...
			if ( typeof col.render != 'undefined') {
				if ( typeof col.render == 'function') {
					...
					if (data.length >= 4 && data.substr(0, 4) != '<div')
						data = '<div>' + data + '</div>';
				}
				if ( typeof col.render == 'object')
					data = '<div>' + col.render[data] + '</div>';
				if ( typeof col.render == 'string') {
					var tmp = col.render.toLowerCase().split(':');
					var prefix = '';
					var suffix = '';
			...
			if ( typeof msg != 'undefined') {
				...
				if (sel.length > 0) {
					...
					if ( typeof tmp == 'object')
						tmp = tmp.recid + ', ' + w2utils.lang('Column') + ': ' + tmp.column;
					if (sel.length == 1)
		...
		unlock : function() {
			...
			// needed timer so if server fast, it will not flash
		},
...
(function() {
		...
		if ( typeof method === 'object' || !method) {
			...
			for (var p in {
				...
				if (object.get(p) != null)
					continue;
				object.panels[p] = $.extend(true, {}, w2layout.prototype.panel, {
					type : p,
					hidden : true,
					size : 50
				});
	...
	w2layout.prototype = {
		...
		set : function(panel, options) {
			...
			this.refresh(panel);
			this.resize();
			// resize is needed when panel size is changed
			return true;
		},
		...
		render : function(box) {
			...
			obj.refresh();
			// if refresh is not called here, the layout will not be available right after initialization
			// process event
			obj.trigger($.extend(eventData, {
				phase : 'after'
			}));
		...
		refresh : function(panel) {
			...
			if (panel != null && typeof panel != 'undefined') {
				...
				var el = $('#layout_' + obj.name + '_panel_' + panel).css({
		...
		resize : function() {
			...
			for (var p in {
				...
				var height = 0;
				if (pan.show.tabs) {
					if (pan.tabs != null && w2ui[this.name + '_' + p + '_tabs'])
						w2ui[this.name + '_' + p + '_tabs'].resize();
					height += w2utils.getSize($(tmp + 'tabs').css({
						display : 'block'
					}), 'height');
				}
				if (pan.show.toolbar) {
					if (pan.toolbar != null && w2ui[this.name + '_' + p + '_toolbar'])
						w2ui[this.name + '_' + p + '_toolbar'].resize();
					height += w2utils.getSize($(tmp + 'toolbar').css({
						top : height + 'px',
						display : 'block'
					}), 'height');
				}
...
(function() {
	...
	$.fn.w2popup = function(method, options) {
		...
		if (method == 'open' && typeof options.url != 'undefined')
			method = 'load';
		...
		if ($(this).length > 0) {
			...
			if (parseInt($(this).css('width')) != 0)
				dlgOptions['width'] = parseInt($(this).css('width'));
			if (parseInt($(this).css('height')) != 0)
				dlgOptions['height'] = parseInt($(this).css('height'));
		}
		// show popup
	...
	w2popup = {
		...
		open : function(options) {
			...
			if (options.onOpen)
				w2popup.onOpen = options.onOpen;
			if (options.onClose)
				w2popup.onClose = options.onClose;
			if (options.onMax)
				w2popup.onMax = options.onMax;
			if (options.onMin)
				w2popup.onMin = options.onMin;
			if (options.onKeydown)
				w2popup.onKeydown = options.onKeydown;
			...
			if (parseInt(width) - 10 < parseInt(options.width))
		...
		load : function(options) {
			...
			if ( typeof html != 'undefined' && html != null) {
				...
				$.get(url, function(data, status, obj) {
					...
					// remember for possible future purposes
				});
			}
		...
		message : function(options) {
			...
			if (parseInt(options.width) < 10)
				options.width = 10;
			if (parseInt(options.height) < 10)
				options.height = 10;
			...
			if ($('#w2ui-popup .w2ui-popup-message').length == 0) {
				...
				if ( typeof options.width == 'undefined')
					options.width = w2utils.getSize($('#w2ui-popup .w2ui-popup-message'), 'width');
				if ( typeof options.height == 'undefined')
					options.height = w2utils.getSize($('#w2ui-popup .w2ui-popup-message'), 'height');
			}
		...
		lockScreen : function(options) {
			if ($('#w2ui-lock').length > 0)
				return false;
			if ( typeof options == 'undefined')
				options = $('#w2ui-popup').data('options');
			if ( typeof options == 'undefined')
				options = {};
		...
		resize : function(width, height, callBack) {
			...
			if (parseInt($(window).width()) - 10 < parseInt(width))
				width = parseInt($(window).width()) - 10;
			if (parseInt($(window).height()) - 10 < parseInt(height))
...
var w2alert = function(msg, title, callBack) {
	...
	if ($('#w2ui-popup').length > 0) {
		w2popup.message({
			width : 400,
			height : 150,
			html : '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">' + '		<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + '</div></div>' + '</div>' + '<div style="position: absolute; bottom: 7px; left: 0px; right: 0px; text-align: center; padding: 5px">' + '		<input type="button" value="Ok" onclick="w2popup.message();" class="w2ui-popup-button">' + '</div>',
			onClose : function() {
				if ( typeof callBack == 'function')
					callBack();
			}
...
var w2confirm = function(msg, title, callBack) {
	...
	if ($('#w2ui-popup').length > 0) {
		w2popup.message({
			width : 400,
			height : 150,
			html : '<div style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 40px; overflow: auto">' + '		<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + '</div></div>' + '</div>' + '<div style="position: absolute; bottom: 7px; left: 0px; right: 0px; text-align: center; padding: 5px">' + '		<input id="No" type="button" value="' + w2utils.lang('No') + '" class="w2ui-popup-button">' + '		<input id="Yes" type="button" value="' + w2utils.lang('Yes') + '" class="w2ui-popup-button">' + '</div>',
			onOpen : function() {
		...
		w2popup.open({
			...
			showClose : false,
			body : '<div class="w2ui-centered"><div style="font-size: 13px;">' + msg + '</div></div>',
			buttons : '<input id="No" type="button" value="' + w2utils.lang('No') + '" class="w2ui-popup-button">' + '<input id="Yes" type="button" value="' + w2utils.lang('Yes') + '" class="w2ui-popup-button">',
...
(function() {
	...
	w2tabs.prototype = {
		...
		select : function(id) {
			if (this.get(id) == null || this.active == id)
				return false;
			this.active = id;
			this.refresh();
			return true;
		...
		refresh : function(id) {
			...
			if (tab == null)
				return;
			if ( typeof tab.caption != 'undefined')
				tab.text = tab.caption;
...
(function() {
	var w2toolbar = function(options) {
		...
		this.items = [], this.right = '', // HTML text on the right of toolbar
		this.onClick = null, this.onRender = null, this.onRefresh = null, this.onResize = null, this.onDestroy = null
		...
		getItemHTML : function(item) {
			...
			switch (item.type) {
				...
				case 'drop':
					var img = '<td>&nbsp;</td>';
					if (item.img)
						img = '<td><div class="w2ui-tb-image w2ui-icon ' + item.img + '"></div></td>';
					if (item.icon)
						img = '<td><div class="w2ui-tb-image"><span class="' + item.icon + '"></span></div></td>';
			...
			if ( typeof item.onRender == 'function')
				newHTML = item.onRender.call(this, item.id, html);
			if ( typeof this.onRender == 'function')
				newHTML = this.onRender(item.id, html);
			if (newHTML != '' && typeof newHTML != 'undefined')
				html = newHTML;
			return html;
		},
		...
		menuClick : function(id, menu_index, event) {
			...
			if (it && !it.disabled) {
				...
				var eventData = this.trigger({
					...
					target : ( typeof id != 'undefined' ? id : this.name),
					item : this.get(id),
					subItem : ( typeof menu_index != 'undefined' && this.get(id) ? this.get(id).items[menu_index] : null),
					originalEvent : event
				});
				if (eventData.isCancelled === true)
					return false;
			...
			if (it && !it.disabled) {
				...
				if (it.type == 'radio') {
					for (var i = 0; i < this.items.length; i++) {
						...
						if (itt == null || itt.id == it.id || itt.type != 'radio')
							continue;
...
(function() {
	var w2sidebar = function(options) {
		...
		this.menu = [];
		this.selected = null;
		// current selected node (readonly)
		this.img = null;
		this.icon = null;
		this.style = '';
		this.topHTML = '';
		...
		this.keyboard = true;
		this.onClick = null;
		// Fire when user click on Node Text
		this.onDblClick = null;
		// Fire when user dbl clicks
		this.onContextMenu = null;
		this.onMenuClick = null;
		// when context menu item selected
		this.onExpand = null;
		// Fire when node Expands
		this.onCollapse = null;
		// Fire when node Colapses
		this.onKeydown = null;
		this.onRender = null;
		this.onRefresh = null;
		this.onResize = null;
	...
	w2sidebar.prototype = {
		...
		insert : function(parent, before, nodes) {
			...
			for (var o in nodes) {
				...
				var nd = tmp.nodes;
				tmp.nodes = [];
				// very important to re-init empty nodes array
		...
		click : function(id, event) {
			...
			if (nd.disabled || nd.group)
				return;
			// should click event if already selected
			// move selected first
		...
		keydown : function(event) {
			...
			function next(node, noSubs) {
				...
				if (nextNode != null && (nextNode.disabled || nextNode.group))
			...
			function prev(node) {
				...
				if (prevNode != null && (prevNode.disabled || prevNode.group))
		...
		resize : function() {
			...
			// container should have no overflow
	...
	$.extend(w2field, {
			...
			return $(this).each(function(field, index) {
				...
				switch (tp) {
					...
					case 'hex':
						...
						// init events
						$(this).data('options', options).on('keypress', function(event) {// keyCode & charCode differ in FireFox
							...
							// check validity
							if (this.value != '' && !checkType(this.value))
								$(this).val(options.min != null ? options.min : '');
						});
						if ($(this).val() == '' && options.min != null)
						...
						if (options.arrows != '') {
							...
							helper.css({
							...
							}).css('margin-left', '-' + (helper.width() + parseInt($(this).css('margin-right')) + 12) + 'px').on('mousedown', function(event) {
								var btn = this;
								var evt = event;
								$('body').on('mouseup', tmp);
								...
								function update(notimer) {
									...
									if (notimer !== false)
										$('body').data('_field_update_timer', setTimeout(update, 60));
								};
							});
							pr += helper.width() + 12;
					...
					case 'date':
						...
						.css({
							...
							'-ms-transition' : 'none',
							'-o-transition' : 'none'
						}).data("options", options).on('focus', function() {
							...
							var mtimer = setInterval(function() {
								...
								if (left + $('#global_calendar_div').width() > max)
									left = max - $('#global_calendar_div').width();
								// monitor if moved
					...
					case 'color':
						...
						$(this).attr('maxlength', 6).on('focus', function() {
							...
							$('#global_color_div').html($().w2field('getColorHTML', obj.value)).css({
								left : left + 'px',
								top : top + 'px'
							}).data('el', obj).show();
							var max = $(window).width() + $(document).scrollLeft() - 1;
							...
							var mtimer = setInterval(function() {
								...
								var left = $(obj).offset().left;
								if (left + $('#global_color_div').width() > max)
									left = max - $('#global_color_div').width();
								// monitor if moved
					...
					case 'list':
						...
						obj.refresh = function() {
							...
							if (settings.showNone)
								html = '<option value="">- ' + w2utils.lang('none') + ' -</option>';
							for (var i in items) {
								if (!settings.showNone && settings.value == null)
									settings.value = items[i].id;
					...
					case 'enum':
						...
						this.show = function() {
							...
							}).width(w2utils.getSize(obj, 'width')).data('position', ($(obj).offset().left) + 'x' + ($(obj).offset().top + obj.offsetHeight));
							...
							if ( typeof settings.onShow == 'function')
								settings.onShow.call(this, settings);
						}
						...
						this.hide = function() {
							...
							if ( typeof settings.onHide == 'function')
								settings.onHide.call(this, settings);
						}
						...
						this.resize = function() {
							...
							//w2utils.getSize(div, 'height');
							if (cntHeight < 23)
								cntHeight = 23;
							if (cntHeight > settings.maxHeight)
								cntHeight = settings.maxHeight;
					...
					case 'upload':
						...
						if (settings.base64 === true) {
							if (settings.maxSize == 0)
								settings.maxSize = 20 * 1024 * 1024;
							// 20mb
							if (settings.maxFileSize == 0)
								settings.maxFileSize = 20 * 1024 * 1024;
							// 20mb
						}
						...
						this.refresh = function() {
							...
							for (var s in selected) {
								...
								li.data('file', file).on('click', function(event) {
									...
									$(this).w2overlay(previewHTML.replace('##FILE##', (file.content ? 'data:' + file.type + ';base64,' + file.content : '')), {
										top : -4
									});
		...
		cleanItems : function(items) {
			...
			for (var i in items) {
				...
				if (w2utils.isInt(id))
					id = parseInt(id);
				if (w2utils.isFloat(id))
					id = parseFloat(id);
				newItems.push($.extend({}, opt, {
					id : id,
					text : text
				}));
			}
		...
		upload_init : function() {
			...
			var el = $(obj).prev();
			if (el.length > 0 && el[0].tagName == 'DIV' && el.hasClass('w2ui-upload'))
				el.remove();
			// rebuild it
			...
			$(div).off('click').on('click', function(event) {
				...
				if (event.target.tagName != 'INPUT') {
					...
					if (cnt < settings.max || settings.max == 0)
		...
		list_render : function(search) {
			...
			if ( typeof $(obj).data('last_index') == 'undefined' || $(obj).data('last_index') == null)
				$(obj).data('last_index', 0);

			// pull items from url
			if ( typeof settings.last_total == 'undefined')
				settings.last_total = -1;
			if ( typeof settings.last_search_len == 'undefined')
				settings.last_search_len = 0;
			if ( typeof settings.last_search_match == 'undefined')
				settings.last_search_match = -1;
			if (settings.url != '' && ((items.length == 0 && settings.last_total != 0) || (search.length > settings.last_search_len && settings.last_total > settings.maxCache) || (search.length < settings.last_search_match && search.length != settings.last_search_len)
				...
				var match = false;
				if (settings.last_total < settings.maxCache)
					match = true;
				$.ajax({
					type : 'GET',
					dataType : 'text',
			...
			ids.push(w2utils.isInt(selected[a].id) ? parseInt(selected[a].id) : String(selected[a].id))
			// build list
			var group = '';
			for (var a in items) {
				var id = items[a].id;
				...
				// if already selected
				if ($.inArray(w2utils.isInt(id) ? parseInt(id) : String(id), ids) != -1 && settings.showAll !== true)
					continue;
				...
				if (settings.match.toLowerCase() == 'contains' && txt2.indexOf(txt1) != -1)
					match = true;
				if (match) {
					...
					if (txt !== false) {
						...
						if (i == $(obj).data('last_index'))
							$(obj).data('last_item', items[a]);
						i++;
					}
				}
			}
			...
			if (div.find('li.selected').length > 0)
				div.find('li.selected')[0].scrollIntoView(false);
				...
				setTimeout(function() {
					...
					switch (event.keyCode) {
						...
						case 40:
							// down
							curr++;
							if (curr > $(obj).data('last_max'))
								curr = $(obj).data('last_max');
							$(obj).data('last_index', curr);
							if (event.preventDefault)
								event.preventDefault();
							break;
						case 13:
							// enter
							if ( typeof $(obj).data('last_item') == 'undefined' || $(obj).data('last_item') == null || noItems === true)
								break;
							var selected = $(obj).data('selected');
							obj.add($(obj).data('last_item'));
							// select next
							if (curr > $(obj).data('last_max') - 1)
								curr = $(obj).data('last_max') - 1;
							$(obj).data('last_index', curr);
		...
		calendar_get : function(date, options) {
			...
			if (String(date) == '' || String(date) == 'undefined')
				date = w2utils.formatDate(today, options.format);
			if (!w2utils.isDate(date, options.format))
				date = w2utils.formatDate(today, options.format);
			// format date
			...
			var dt = new Date();
			if (tmp2 == 'mm/dd/yyyy')
				dt = new Date(tmp[0] + '/' + tmp[1] + '/' + tmp[2]);
			if (tmp2 == 'm/d/yyyy')
				dt = new Date(tmp[0] + '/' + tmp[1] + '/' + tmp[2]);
			if (tmp2 == 'dd/mm/yyyy')
				dt = new Date(tmp[1] + '/' + tmp[0] + '/' + tmp[2]);
			if (tmp2 == 'd/m/yyyy')
				dt = new Date(tmp[1] + '/' + tmp[0] + '/' + tmp[2]);
			if (tmp2 == 'yyyy/dd/mm')
				dt = new Date(tmp[2] + '/' + tmp[1] + '/' + tmp[0]);
			if (tmp2 == 'yyyy/d/m')
				dt = new Date(tmp[2] + '/' + tmp[1] + '/' + tmp[0]);
			if (tmp2 == 'yyyy/mm/dd')
				dt = new Date(tmp[1] + '/' + tmp[2] + '/' + tmp[0]);
			if (tmp2 == 'yyyy/m/d')
				dt = new Date(tmp[1] + '/' + tmp[2] + '/' + tmp[0]);
			if (tmp2 == 'mm/dd/yy')
				dt = new Date(tmp[0] + '/' + tmp[1] + '/' + (parseInt(tmp[2]) + 1900));
			if (tmp2 == 'm/d/yy')
				dt = new Date(tmp[0] + '/' + tmp[1] + '/' + (parseInt(tmp[2]) + 1900));
			if (tmp2 == 'dd/mm/yy')
				dt = new Date(tmp[1] + '/' + tmp[0] + '/' + (parseInt(tmp[2]) + 1900));
			if (tmp2 == 'd/m/yy')
				dt = new Date(tmp[1] + '/' + tmp[0] + '/' + (parseInt(tmp[2]) + 1900));
			if (tmp2 == 'yy/dd/mm')
				dt = new Date(tmp[2] + '/' + tmp[1] + '/' + (parseInt(tmp[0]) + 1900));
			if (tmp2 == 'yy/d/m')
				dt = new Date(tmp[2] + '/' + tmp[1] + '/' + (parseInt(tmp[0]) + 1900));
			if (tmp2 == 'yy/mm/dd')
				dt = new Date(tmp[1] + '/' + tmp[2] + '/' + (parseInt(tmp[0]) + 1900));
			if (tmp2 == 'yy/m/d')
				dt = new Date(tmp[1] + '/' + tmp[2] + '/' + (parseInt(tmp[0]) + 1900));
			var html = '<table cellpadding="0" cellspacing="0"><tr>' + '<td>' + $().w2field('calendar_month', (dt.getMonth() + 1), dt.getFullYear(), options) + '</td>' +
		...
		calendar_month : function(month, year, options) {
			...
			month = Number(month);
			if (year === null || year === '')
				year = String(td.getYear()).length > 3 ? td.getYear() : td.getYear() + 1900;
			if (month === null || month === '')
				month = Number(td.getMonth()) + 1;
			if (month > 12) {
				month = month - 12;
				year++;
			}
			...
			for (var ci = 1; ci < 43; ci++) {
				...
				var className = '';
				if (ci % 7 == 6)
					className = 'w2ui-saturday';
				if (ci % 7 == 0)
					className = 'w2ui-sunday';
				if (dt == today)
					className += ' w2ui-today';
				...
				var blocked = '';
				if (options.colored)
					if (options.colored[dt] != undefined) {// if there is predefined colors for dates
...
(function() {
	var w2form = function(options) {
		...
		this.url = '';
		this.formURL = '';
		// url where to get form HTML
		this.formHTML = '';
		// form HTML (might be loaded from the url)
		this.page = 0;
		// current page
		this.recid = 0;
		// can be null or 0
		...
		this.postData = {};
		this.toolbar = {};
		// if not empty, then it is toolbar
		this.tabs = {};
		// if not empty, then it is tabs object

		this.style = '';
		this.focus = 0;
		// focus first or other element
	...
	$.fn.w2form = function(method) {
		if ( typeof method === 'object' || !method) {
			...
			// reassign variables
			for (var p in fields)
			object.fields[p] = $.extend(true, {}, fields[p]);
			for (var p in record) {
	...
	w2form.prototype = {
		...
		validate : function(showErrors) {
			...
			for (var f in this.fields) {
				...
				switch (field.type) {
					...
					case 'checkbox':
						// convert true/false
						if (this.record[field.name] == true)
							this.record[field.name] = 1;
						else
							this.record[field.name] = 0;
						break;
					case 'date':
		...
		request : function(postData, callBack) {// if (1) param then it is call back if (2) then postData and callBack
			...
			if ( typeof postData == 'undefined' || postData == null)
				postData = {};
			if (!this.url || ( typeof this.url == 'object' && !this.url.get))
				return;
			if (this.recid == null || typeof this.recid == 'undefined')
				this.recid = 0;
			// build parameters list
			var params = {};
			// add list params
			...
			if ( typeof eventData.url == 'object' && eventData.url.get)
				url = eventData.url.get;
			if (this.last.xhr)
				try {
					this.last.xhr.abort();
				} catch (e) {
			...
			setTimeout(function() {
				...
				if (eventData.isCancelled === true) {
					if ( typeof callBack == 'function')
						callBack({
							status : 'error',
							message : 'Saving aborted.'
						});
					return false;
				}
				// default action
				var url = eventData.url;
				if ( typeof eventData.url == 'object' && eventData.url.save)
					url = eventData.url.save;
				if (obj.last.xhr)
					try {
						obj.last.xhr.abort();
		...
		goto : function(page) {
			...
			if ($(this.box).data('auto-size') === true)
				$(this.box).height(0);
			this.refresh();
		},
		...
		generateHTML : function() {
			...
			for (var f in this.fields) {
				...
				if (field.html.caption == '')
					field.html.caption = field.name;
				var input = '<input name="' + field.name + '" type="text" ' + field.html.attr + '/>';
				if (field.type == 'list')
					input = '<select name="' + field.name + '" ' + field.html.attr + '></select>';
				...
				if ( typeof pages[field.html.page] == 'undefined')
					pages[field.html.page] = '<div class="w2ui-page page-' + field.html.page + '">';
		...
		resize : function() {
			...
			if (parseInt($(this.box).height()) == 0 || $(this.box).data('auto-size') === true) {
				$(this.box).height((header.length > 0 ? w2utils.getSize(header, 'height') : 0) + (this.tabs.tabs.length > 0 ? w2utils.getSize(tabs, 'height') : 0) + (this.toolbar.items.length > 0 ? w2utils.getSize(toolbar, 'height') : 0) + (page.length > 0 ? w2utils.getSize(dpage, 'height') + w2utils.getSize(cpage, '+height') + 12 : 0) + // why 12 ???
		...
		refresh : function() {
			...
			if (!this.box)
				return;
			if (!this.isGenerated || typeof $(this.box).html() == 'undefined')
				return;
				...
				$(field.el).w2field('clear').off('change').on('change', function() {
					...
					if ((field.type == 'enum' || field.type == 'upload') && $(this).data('selected')) {
						...
						if ($.isArray(new_arr))
							...
							value_new[i] = $.extend(true, {}, new_arr[i]);
						// clone array
						if ($.isArray(cur_arr))
							for (var i in cur_arr)
							value_previous[i] = $.extend(true, {}, cur_arr[i]);
						// clone array
					}
					// event before
					var eventData = obj.trigger({
				...
				switch (String(field.type).toLowerCase()) {
					...
					case 'date':
						...
						if (!field.options.format)
							field.options.format = w2utils.settings.date_format;
						field.el.value = value;
			...
			for (var i = 0; i < tmp.length; i++) {
				if ($(tmp[i]).find('> *').length > 1)
					$(tmp[i]).wrapInner('<div></div>');
			}
			// event after
			this.trigger($.extend(eventData, {
				phase : 'after'
		...
		render : function(box) {
			...
			// need timer because resize is on timer
			// focus on load
			function focusEl() {
				var inputs = $(obj.box).find('input, select, textarea');
				if (inputs.length > obj.focus)
					inputs[obj.focus].focus();
			}

			if (this.focus >= 0)
				setTimeout(focusEl, 500);
			// need timeout to allow form to render
		},
		...
		destroy : function() {
			...
			if ( typeof this.toolbar == 'object' && this.toolbar.destroy)
				this.toolbar.destroy();
			if ( typeof this.tabs == 'object' && this.tabs.destroy)
				this.tabs.destroy();
			if ($(this.box).find('#form_' + this.name + '_tabs').length > 0) {
				$(this.box).removeAttr('name').removeClass('w2ui-reset w2ui-form').html('');
			}
